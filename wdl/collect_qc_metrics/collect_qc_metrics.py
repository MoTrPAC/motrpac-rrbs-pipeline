"""
Authors:
    Samir Akre, Archana Raja
Purpose:
    The goal of this script is to aggregate QC metrics for the motrpac RRBS initial
    processing pipeline. Read the usage() function for details on usage.
Output:
    writes csv file: '${SID}_qcmetrics.csv' where ${SID} is based on the sample ID sent
    as input with -u or --sid
Usage:
    python3 collect_qc_metrics.py \
        --summary reports/bismark_summary_report.txt \
        --bt2 reports/Rat_Muscle_attached_R1_val_1.fq_trimmed_bismark_bt2_PE_report.txt \
        --lambda_summary reports/lambda/bismark_summary_report.txt \
        --multiqc reports/multiQC_report/multiqc_data/multiqc_general_stats.txt \
        --tg reports/Rat_Muscle_trim.log --td reports/Rat_Muscle_trimDiversityAdapt.log \
        --phix_report reports/Rat_Muscle_phix_report.txt \
        --mapped_report reports/Rat_Muscle_mapped_report.txt
Notes :
    Any of the function can throw an error on the script like a variable is used before
    it's being declared, if the report it reads is empty, have to add a check to throw
...
def parse_trim_galore_log(tglog):
    l = []
    data = {}
...
def parse_trim_diversity_log(tdlog):
    data = {}
    with open(tdlog) as trim_log:
        for line in trim_log:
            if "Fwd:  D0:" in line:
                l = re.split(r"other=|total=|\(|\)", line)
                other = int(l[-4])
                total = int(l[-2])
...
def parse_bismark_summary(bismark_summary):
    """
    Parse bismark_summary_report.txt file for RRBS QC metrics

    :param bismark_summary: path to file containing output from bismark2summary
    :return: DataFrame
    """
    report_table = pd.read_csv(bismark_summary, delimiter="\t")
    # for col in report_table.columns:
    #    print(col, ': ', report_table[col].values[0])
    data = {}
    unMetCs = (
        report_table["Unmethylated CpGs"]
        + report_table["Unmethylated chgs"]
        + report_table["Unmethylated CHHs"]
    )
    ...
    data["pct_Aligned"] = [
        100 * (report_table["Aligned Reads"] / report_table["Total Reads"]).values[0]
    ]
    data["pct_Unaligned"] = [
        100 * (report_table["Unaligned Reads"] / report_table["Total Reads"]).values[0]
    ]
    data["pct_Ambi"] = [
        100
        * (
            report_table["Ambiguously Aligned Reads"] / report_table["Total Reads"]
        ).values[0]
    ]
    data["pct_CpG"] = [
        100
        * (
            report_table["Methylated CpGs"]
            / (report_table["Unmethylated CpGs"] + report_table["Methylated CpGs"])
        ).values[0]
    ]
    data["pct_CHG"] = [
        100
        * (
            report_table["Methylated chgs"]
            / (report_table["Unmethylated chgs"] + report_table["Methylated chgs"])
        ).values[0]
    ]
    data["pct_CHH"] = [
        100
        * (
            report_table["Methylated CHHs"]
            / (report_table["Unmethylated CHHs"] + report_table["Methylated CHHs"])
        ).values[0]
    ]
    data["pct_umi_dup"] = [
        100
        * (
            report_table["Duplicate Reads (removed)"] / report_table["Aligned Reads"]
        ).values[0]
    ]
    return pd.DataFrame(data=data)


def get4_strand_map_data(bismark_report):
    """
    ...
    totalAligned = (
        data["pct_OT"][0] + data["pct_CTOT"][0] + data["pct_CTOB"][0] + data["pct_OB"][0]
    )
...
def parse_multi_qc_report(multiqc_general_stats):
    data = {}
    stat_table = pd.read_csv(multiqc_general_stats, delimiter="\t")
    seq_count = stat_table["FastQC_mqc-generalstats-fastqc-total_sequences"]
    data["reads_raw"] = int((seq_count[2] + seq_count[3]) / 2)
    pct_trimmed_r1 = 100 * (1 - seq_count[4] / seq_count[2])
    pct_trimmed_r2 = 100 * (1 - seq_count[5] / seq_count[3])
    data["pct_trimmed"] = round((pct_trimmed_r1 + pct_trimmed_r2) / 2, 3)
    print(data["pct_trimmed"])
    data["reads"] = int((seq_count[4] + seq_count[5]) / 2)
    print(data["reads"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to collect RRBS qc metrics. \
    ...
    parser.add_argument(
        "--summary", help="path to bismark2summary alignment report to species"
    )
    parser.add_argument(
        "--bt2", help="path to bowtie2 paired end alignment report to species"
    )
    parser.add_argument(
        "--lambda_summary", help="path to bismark2summary alignment report to lambda"
    )
    parser.add_argument(
        "--multiqc", help="path to multiQC report generated on pre and post trimmed reads"
    )
    ...
    fourStrandData = get4_strand_map_data(args.bt2)
    spikeInSummary = parse_bismark_summary(args.lambda_summary)
    multiQCData = parse_multi_qc_report(args.multiqc)
