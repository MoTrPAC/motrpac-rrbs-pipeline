...
WARNING: This version of the script as been modified to allow output directory 
specification used the -d flag

Edited by Samir Akre
"""
...
try:
    optlist, args = getopt(sys.argv[1:], "h1:2:o:y:d:tb")
except:
...
for (opt, opt_arg) in optlist:
    if opt == "-h":
        ...
        outDir = opt_arg
        print("WARNING: output directory flag set, this is an untested feature")
...
for i in range(len(fwdFiles)):
    ...
    if revFilenames:
        ...
        if outDir:
            revRoot = outDir+'/'+os.path.basename(revRoot)
    ...
    if outDir:
        fwdRoot = outDir+'/'+os.path.basename(fwdRoot)
